...
def segment_image(grayscale, peaks_indexes):
    segmented_img = np.copy(grayscale)
    for ri in range(segmented_img.shape[0]):
        if ri in peaks_indexes:
            segmented_img[ri, :] = 0
    return segmented_img

def group_peaks(peaks_indexes):
...
def process_image(image_path, processor, model):
    ...
    peak_groups = group_peaks(peaks_indexes)
    ...
    for index, sub_image_index in enumerate(peak_groups):
        ...
        page_text = page_text + '\n' + ocr_image(sub_image, processor, model)
